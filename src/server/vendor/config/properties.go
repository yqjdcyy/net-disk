...
// GetStringDefault gets the string value for the given option in the section.
// If the value needs to be unfolded (see e.g. %(host)s example in the
// beginning of this documentation),
// then GetString does this unfolding automatically, up to DepthValues number
// of iterations.
// It returns an error if either the section or the option do not exist, or
// the unfolding cycled.
func (c *ConfigFile) GetStringDefault(section string, option string, defaultVal string) string {
	var val string
	var e error
	if val, e = c.GetString(section, option); e != nil {
		val = defaultVal
	}
	return val
}

// GetInt64Default has the same behaviour as GetString but converts the response to int.
func (c *ConfigFile) GetInt64Default(section string, option string, defaultVal int64) int64 {
	var val int64
	var e error
	if val, e = c.GetInt64(section, option); e != nil {
		val = defaultVal
	}
	return val
}

// GetIntDefault has the same behaviour as GetString but converts the response to int.
func (c *ConfigFile) GetIntDefault(section string, option string, defaultVal int) int {
	var val int
	var e error
	if val, e = c.GetInt(section, option); e != nil {
		val = defaultVal
	}
	return val
}

// GetFloatDefault has the same behaviour as GetString but converts the response to
// float.
func (c *ConfigFile) GetFloatDefault(section string, option string, defaultVal float64) float64 {

	var val float64
	var e error
	if val, e = c.GetFloat(section, option); e != nil {
		val = defaultVal
	}
	return val
}

// GetBoolDefault has the same behaviour as GetString but converts the response to
// bool.
// See constant BoolStrings for string values converted to bool.
func (c *ConfigFile) GetBoolDefault(section string, option string, defaultVal bool) bool {

	var val bool
	var e error
	if val, e = c.GetBool(section, option); e != nil {
		val = defaultVal
	}
	return val
}
