...
func SetLogPath(path string) {
	...
	if 0==len(path){
		log.Panic("please set the log.path")
	}
	file, err := utils.OpenOrCreate(path)
	if nil != err {
		log.Panicf("fail to open or crate file[%v]: %v", path, err.Error())
	}
	logger.file = file
	logger.printer = log.New(file, "[client]", log.LstdFlags)
	log.Printf("Statistic.Logger.init bind to file[%v]", path)
}
...
func Close() {
	...
	if nil != logger.file {
		...
		if nil != err {
			log.Panicf("fail to close log file: %v", err.Error())
		}
	}
}
